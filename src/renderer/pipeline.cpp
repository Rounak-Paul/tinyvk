...
bool Pipeline::CreateFullscreenQuad(Renderer* renderer, VkRenderPass renderPass, const std::string& vertShaderSource, const std::string& fragShaderSource, VkDescriptorSetLayout descriptorSetLayout, const std::vector<VkPushConstantRange>& pushConstants) {
    _renderer = renderer;
    auto& ctx = renderer->GetContext();
    VkDevice device = ctx.GetDevice();

    VkShaderModule vertShaderModule = ShaderCompiler::CreateShaderModuleFromGLSL(
        renderer, vertShaderSource, ShaderStage::Vertex, "fullscreen.vert"
    );
    VkShaderModule fragShaderModule = ShaderCompiler::CreateShaderModuleFromGLSL(
        renderer, fragShaderSource, ShaderStage::Fragment, "fullscreen.frag"
    );
    
    if (vertShaderModule == VK_NULL_HANDLE || fragShaderModule == VK_NULL_HANDLE) {
        TVK_LOG_ERROR("Failed to create fullscreen quad shader modules");
        return false;
    }
    ...
    // No vertex input for fullscreen quad - vertices are generated in shader using gl_VertexIndex
    VkPipelineVertexInputStateCreateInfo vertexInputInfo{};
    vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
    ...
    // No depth testing for fullscreen quad
    VkPipelineDepthStencilStateCreateInfo depthStencil{};
    depthStencil.sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
    ...
    // Handle Push Constants
    std::vector<VkPushConstantRange> ranges = pushConstants;
    ...
    if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &_layout) != VK_SUCCESS) {
        TVK_LOG_ERROR("Failed to create fullscreen quad pipeline layout");
        vkDestroyShaderModule(device, vertShaderModule, nullptr);
        vkDestroyShaderModule(device, fragShaderModule, nullptr);
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &_pipeline) != VK_SUCCESS) {
        TVK_LOG_ERROR("Failed to create fullscreen quad graphics pipeline");
        vkDestroyShaderModule(device, vertShaderModule, nullptr);
        vkDestroyShaderModule(device, fragShaderModule, nullptr);
        return false;
    }
    ...
    TVK_LOG_INFO("Fullscreen quad pipeline created successfully");
    return true;
}
